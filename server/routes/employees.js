...
router.get('/', (req, res) => {
  ...
  if (user.role === 'employee') {
    ...
    const emp = db.prepare('SELECT * FROM employees WHERE id = ?').get(user.employee_id);
...
router.post('/', authorize('admin', 'hr'), (req, res) => {
  ...
  const id = uuidv4();
  try {
    db.prepare('INSERT INTO employees (id, employee_id, first_name, last_name, email, phone, department, position, salary, join_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
...
router.put('/:id', authorize('admin', 'hr'), (req, res) => {
  ...
  if (!emp) return res.status(404).json({ error: 'Employee not found' });
  try {
    db.prepare('UPDATE employees SET first_name=?, last_name=?, email=?, phone=?, department=?, position=?, salary=?, join_date=?, status=? WHERE id=?')
      .run(first_name||emp.first_name, last_name||emp.last_name, email||emp.email,
           phone!==undefined?phone:emp.phone, department||emp.department, position||emp.position,
           salary||emp.salary, join_date||emp.join_date, status||emp.status, req.params.id);
