...
router.post('/', (req, res) => {
  ...
  if (existing) {
    db.prepare(`
      UPDATE attendance SET check_in=?, check_out=?, hours_worked=?, status=? WHERE id=?
    `).run(check_in || existing.check_in, check_out || existing.check_out,
           hours_worked || existing.hours_worked, status || existing.status, existing.id);
    return res.json({ message: 'Attendance updated' });
  }

  const id = uuidv4();
  db.prepare(`
    INSERT INTO attendance (id, employee_id, date, check_in, check_out, hours_worked, status)
    VALUES (?, ?, ?, ?, ?, ?, ?)
